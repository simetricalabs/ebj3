...
class LiveUpdateDownloadHelper
{
	 ...
	 */
	public static function download($url, $target, &$errorMsg)
	{
		...
		if ($fp !== false)
		{
			...
			while (!empty($adapters) && ($result === false) && self::$_errorNum != 22)
			{
	...
	private static function &getCURL($url, $fp = null, $nofollow = false)
	{
		...
		if(empty($result))
		{
			self::$_errorNum = curl_errno($ch);
			self::$_errorMsg = self::$_errorNum . ' - ' . curl_error($ch); 
		}
		curl_close($ch);
