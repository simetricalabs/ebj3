...
function edit_templatepage( $option ) {
	...
	if( $mtconf->get('demo_mode') )
	{
		JFactory::getApplication('site')->redirect( "index.php?option=$option&task=template_pages&template=$template", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function copy_template( $option ) {
	...
	if( $mtconf->get('demo_mode') )
	{
		JFactory::getApplication('site')->redirect( "index.php?option=$option&task=templates", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function copy_template2( $option ) {
	...
	if( $mtconf->get('demo_mode') )
	{
		$app->redirect( "index.php?option=$option&task=templates", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function save_templatepage( $option ) {
	...
	if( $mtconf->get('demo_mode') )
	{
		$app->redirect( "index.php?option=$option&task=template_pages&template=$template", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function rebuild_thumbnails( $option, $cat_id, $limitstart, $limit )
{
	...
	if( $mtconf->get('demo_mode') )
	{
		JFactory::getApplication('site')->redirect( "index.php?option=$option&task=tools", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function config( $option, $show='' ) {
	...
	if( $show == 'all' && $mtconf->get('demo_mode') )
	{
		JFactory::getApplication('site')->redirect( "index.php?option=$option&task=config", JText::_( 'COM_MTREE_THIS_FEATURE_IS_NOT_AVAILABLE_IN_DEMO_MODE' ) );
	}
...
function saveconfig($option, $show) {
	...
	foreach( $post AS $key => $value ) {
		...
		// If site is in demo mode, only allowed config changes for displaye configuration.
		if( $mtconf->get('demo_mode') )
		{
			$sql .= ' AND displayed = 1';
		}
