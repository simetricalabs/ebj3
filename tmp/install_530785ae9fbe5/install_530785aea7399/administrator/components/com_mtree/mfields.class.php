...
class mFieldType_checkbox extends mFieldType {
	...
	function getWhereCondition() {
		$params['searchOperator'] = intval($this->getParam('searchOperator','2'));

		if( $params['searchOperator'] == 1 )
		{
			$searchOperator = 'AND';
		} else {
			$searchOperator = 'OR';
		...
		if( count($where) > 1 ) {
			$return .= '((' . implode(') '.$searchOperator.' (',$where) . '))';
			$return .= ')';
			return $return;
		} else {
			$return .= $where[0] . ')';
...
class mFieldType_tags extends mFieldType {
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		foreach( $rawTags AS $tag => $items )
		{
			...
			$html .= '/>';
			$html .= $tags[$i]->value;
			// $html .= ' <span class="tagcount">('.$tags[$i]->items.')</span>';
			$html .= '</label>';
			$html .= '</li>';
			// $html .= '<br>';
			$i++;
