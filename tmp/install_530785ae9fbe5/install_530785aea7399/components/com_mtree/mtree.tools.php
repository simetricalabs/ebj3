...
function getCatsSelectlist( $cat_id=0, &$cat_tree, $max_level=0 ) {
	$database	= JFactory::getDBO();
	$tmp_mtconf	= mtFactory::getConfig();
...
function getAssignedFieldsID($cat_id) {
	...
	if( !is_null($top_level_cat_id) )
	{
		...
		// Load only published custom fields
		$db->setQuery( "SELECT cf_id FROM #__mt_customfields WHERE cf_id IN (".implode(', ',$cf_id).") AND published = 1" );
		$cf_id = $db->loadColumn();
	}
