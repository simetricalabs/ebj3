...
class Savant2 {
	...
	*/
	...
	{
		global $mtconf;
		if($mtconf->get('template') != 'kinabalu') {
			$this->setPath('template',$mtconf->getjconf('absolute_path').'/components/com_mtree/templates/kinabalu/');
			if($conf['template_path'] != $mtconf->getjconf('absolute_path').'/components/com_mtree/templates/' . $mtconf->get('template') . '/') {
				$this->addPath('template', $mtconf->getjconf('absolute_path').'/components/com_mtree/templates/' . $mtconf->get('template') . '/');
			}
		} elseif($conf['template_path'] != $mtconf->getjconf('absolute_path').'/components/com_mtree/templates/' . $mtconf->get('template') . '/') {
			$this->setPath('template', $mtconf->getjconf('absolute_path').'/components/com_mtree/templates/' . $mtconf->get('template') . '/');
		}
		
		// set the default template search dirs
		if (isset($conf['template_path'])) {
			// user-defined dirs
			$this->addPath('template', $conf['template_path']);
		} else {
			// default directory only
			$this->addPath('template', null);
		}
