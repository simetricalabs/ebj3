...
function savelisting( $option ) {
	...
	else
	{
		...
		# Load field type
		$database->setQuery('SELECT cf_id, field_type, hidden, required_field, published, iscore FROM #__mt_customfields');
		...
		foreach($fieldtype AS $ft) {
			...
			if( $ft->required_field == 1 && in_array($ft->cf_id,$assigned_cf_ids) )
			{
